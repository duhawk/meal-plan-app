...
export default function Reviews() {
    ...
    return (
      <div className="space-y-8">
        <div>
          ...
          <p className="text-text-secondary mt-1 dark:text-gray-400">Feedback from members on all meals.</p>
        </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        {reviews.map((review) => (
          <div key={review.id} className="bg-surface/80 backdrop-blur-lg rounded-xl border border-border-light/50 p-4 transition-all duration-300 hover:shadow-card-hover hover:-translate-y-0.5 dark:bg-slate-800/80 dark:border-slate-700">
            <div className="flex justify-between items-start">
              <div>
                <p className="font-semibold text-text-primary dark:text-white">{review.meal.dish_name}</p>
                <p className="text-sm text-text-secondary dark:text-gray-400">
                  {new Date(review.meal.meal_date).toLocaleDateString()}
                </p>
              </div>
